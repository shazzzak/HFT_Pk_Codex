...
# fixed production strategy for EVERY config: OBI throttle 0.5x/300ms, OFI off,
# soft_inv at its build default (3x clip). Only LATENCY varies across configs.
...
# ---- LATENCY-SENSITIVITY SWEEP (strategy fixed; vary the LatencyModel) ----
# Latency lives in the engine cfg, not in strategy params, so per config we
# monkeypatch H.LatencyModel to a partial of the real class with these kwargs
# (see _process). The send leg (wire_out_*) is the cancel-relevant one -- a slow
# cancel is what lets toxic flow run over a quote you tried to pull.
#   base_45   = current production prior (~45ms, 2% tail 400ms) = CONTROL (thr=0)
#   fast_25   = better connectivity (~25ms, 1% tail 200ms)
#   colo_10   = near-colo (~10ms, 0.5% tail 50ms)
#   slow_85   = degraded (~85ms, 2% tail 400ms)
#   vslow_125 = poor (~125ms, 5% tail 600ms)
#   notail_45 = median 45ms, NO tail spikes (tail_prob=0) -> isolates tail damage
LATENCY_MODES = [
    dict(decision_ms=5.0, wire_out_median_ms=40.0, wire_out_tail_ms=400.0,
         wire_in_median_ms=40.0, wire_in_tail_ms=10.0, tail_prob=0.02),
    dict(decision_ms=5.0, wire_out_median_ms=20.0, wire_out_tail_ms=200.0,
         wire_in_median_ms=20.0, wire_in_tail_ms=10.0, tail_prob=0.01),
    dict(decision_ms=5.0, wire_out_median_ms=5.0,  wire_out_tail_ms=50.0,
         wire_in_median_ms=5.0,  wire_in_tail_ms=5.0,  tail_prob=0.005),
    dict(decision_ms=5.0, wire_out_median_ms=80.0, wire_out_tail_ms=400.0,
         wire_in_median_ms=40.0, wire_in_tail_ms=10.0, tail_prob=0.02),
    dict(decision_ms=5.0, wire_out_median_ms=120.0, wire_out_tail_ms=600.0,
         wire_in_median_ms=40.0, wire_in_tail_ms=10.0, tail_prob=0.05),
    dict(decision_ms=5.0, wire_out_median_ms=40.0, wire_out_tail_ms=0.0,
         wire_in_median_ms=40.0, wire_in_tail_ms=0.0,  tail_prob=0.0),
]
THROTTLE_LABELS = ["base_45", "fast_25", "colo_10", "slow_85", "vslow_125", "notail_45"]
# strategy identical for every config (thr only selects the latency profile).
THROTTLE_MODES = [dict(_OBI) for _ in LATENCY_MODES]
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
# threshold column: the send-leg median latency (the cancel-relevant number).
def _cfg_thr(thr):
    return f"{LATENCY_MODES[thr]['wire_out_median_ms']:g}ms"
...
def _process(args):
    ...
    # LATENCY SWEEP: patch H.LatencyModel to a partial of the REAL class with this
    # config's kwargs so run_symbol_day's LatencyModel(seed=...) builds the swept
    # model. Based on the original mm_backtest class (not a prior patch); re-set
    # every task so configs never leak into each other.
    import functools as _ft
    import mm_backtest as _MB
    H.LatencyModel = _ft.partial(_MB.LatencyModel, **LATENCY_MODES[thr])
    # run
...
def main():
    ...
    ckpt_path = RESULTS / "latency_sweep_CKPT.jsonl"
    ...
    print(f"\nLATENCY -- latency-sensitivity sweep (strategy fixed) (winner frozen "
    ...
    if off_key is not None:
        ...
        print("\n=== control(base_45) - config (PAIRED by day): positive => current-latency wins ===")
    ...
    out = RESULTS / f"latency_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"latency_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"latency_sweep_PERNAME_{stamp}.csv"
