...
def resting_book(snap, t_ms, hit_side):
    # continuous-phase rows only
    c = snap[snap["phase"] == "CONTINUOUS_AUCTION"]
    # nothing to read -> empty book
    if len(c) == 0:
        return [], [], False, np.nan, np.nan
    # ms timestamps for the asof cut
    ts = R.to_ms(c["orig_time"])
    # rows at or before the trade time
    upto = c[ts <= t_ms]
    # no book yet at this time
    if len(upto) == 0:
        return [], [], False, np.nan, np.nan
    # the last snapshot message at/before the trade = the book state hit
    last_seq = upto["msg_seq"].iloc[-1]
    # all rows of that snapshot message = the book state
    book = upto[upto["msg_seq"] == last_seq]
    # explicit-level entry type + the AGG entry type for the hit side
    lvl_type = "OFFER" if hit_side == "BUY" else "BID"
    # the aggregate block beyond L10 on the hit side
    agg_type = "AGG_OFFER" if hit_side == "BUY" else "AGG_BID"
    # explicit levels on the hit side, sorted best-first (offers asc, bids desc)
    lv = book[book["entry_type"] == lvl_type].sort_values(
        "px", ascending=(hit_side == "BUY"))
    # the per-level resting quantities, best first
    level_qtys = lv["qty"].tolist()
    # the per-level prices, best first (for distance-from-touch mapping)
    level_pxs = lv["px"].tolist()
    # is there an AGG block beyond L10 on the hit side?
    has_agg = (book["entry_type"] == agg_type).any()
    # best bid + best ask for the touch benchmark
    bb = book[book["entry_type"] == "BID"]["px"].max()
    ba = book[book["entry_type"] == "OFFER"]["px"].min()
    # (level qtys, level prices, has_agg, best_bid, best_ask), all best-first
    return level_qtys, level_pxs, has_agg, bb, ba
...
def analyse_symbol_day(date, sym, dsets):
    ...
    for (t_ms, hit_side), g in t.groupby(["t_ms", "hit_side"]):
        # total swept quantity across this instant's same-side fills
        swept_qty = float(g["qty"].sum())
        # skip empty/zero
        if swept_qty <= 0:
            continue
        # M1 (trade-distinct): number of DISTINCT trade prices in the cluster,
        # capped at L10+ for comparability
        m1_bucket = min(int(g["price"].nunique()), MAX_LEVEL + 1)
        # the resting book on the side this cluster hit, just before it
        level_qtys, level_pxs, has_agg, best_bid, best_ask = resting_book(
            s, float(t_ms), hit_side)
        # M2 (book-diff): how many resting levels the swept quantity consumed
        m2_bucket = levels_consumed(swept_qty, level_qtys, has_agg)
        # ---- PER-TRADE DISTANCE FROM TOUCH (every fill in this cluster) ----
        # the touch on the side being hit: BUY hits the ASK, SELL hits the BID
        touch = best_ask if hit_side == "BUY" else best_bid
        # for each fill row, which book level its price sat at (distance from touch)
        for _, fl in g.iterrows():
            # the fill price
            px = float(fl["price"])
            # the level index of this fill's price = its distance-from-touch bucket
            dist_bucket = price_level_bucket(px, level_pxs, has_agg)
            ...
            recs.append({"date": date, "symbol": sym, "hit_side": hit_side,
                         ...
                         "fill_qty": float(fl["qty"]), "fill_px": px,
                         "touch": (float(touch) if pd.notna(touch) else np.nan),
...
def plot_hist(tbl_spot, tbl_fut, sym, method, weight, stamp):
    ...
    fig.tight_layout()
    # output path
    p = RESULTS / f"sweepdepth_{sym}_{method}_{weight}_{stamp}.png"
    # save + close
...
def main():
    ...
    # write the tidy CSV
    out.to_csv(RESULTS / f"sweepdepth_{stamp}.csv", index=False)
    print(f"\nwrote {RESULTS / f'sweepdepth_{stamp}.csv'}")
    print(f"wrote PNGs: sweepdepth_<ticker>_<m1|m2>_<count|volume>_{stamp}.png")
