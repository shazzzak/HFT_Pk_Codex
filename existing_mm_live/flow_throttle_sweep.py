...
# ---- AGGRESSOR-FLOW THROTTLE P&L SWEEP ----
# The real test: does acting on the flow signal MAKE MONEY (after the capture we
# forgo by pulling)? Flow uses the 15s half-life the gate picked; we sweep the
# fire threshold (trailing-std) and test flow ALONE vs STACKED on the OBI throttle.
_OBI = dict(obi_throttle=True, ofi_throttle=False, obi_throttle_thresh=0.15,
            ...
            enable_pov_cap=False, flow_throttle=False)
_NONE = dict(obi_throttle=False, ofi_throttle=False, qdr_throttle=False,
             enable_pov_cap=False, flow_throttle=False)
_FLOW_NS = [1.5, 2.0]      # fire thresholds to sweep (trailing std)
# flow config: same 15s half-life, reusing throttle_frac/hold; obi optionally on.
def _flow(n, with_obi):
    base = dict(_OBI) if with_obi else dict(obi_throttle=False, ofi_throttle=False,
                qdr_throttle=False, enable_pov_cap=False,
                throttle_frac=0.5, throttle_hold_ms=300.0)
    base.update(flow_throttle=True, flow_hl_s=15.0, flow_thresh_n=n, flow_std_hl_s=300.0)
    return base
# thr=0 NONE (anchor); thr=1 OBI (current production, the incumbent/control);
# then flow-alone at each N, then flow-stacked-on-OBI at each N.
THROTTLE_MODES = [_NONE, dict(_OBI)]
THROTTLE_LABELS = ["NONE", "OBI"]
for _n in _FLOW_NS:
    THROTTLE_MODES.append(_flow(_n, with_obi=False)); THROTTLE_LABELS.append(f"FLOWonly_n{_n:g}")
for _n in _FLOW_NS:
    THROTTLE_MODES.append(_flow(_n, with_obi=True));  THROTTLE_LABELS.append(f"OBI+FLOW_n{_n:g}")
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
# threshold column: the flow fire threshold (std), or '-' when flow is off.
def _cfg_thr(thr):
    m = THROTTLE_MODES[thr]
    return f"n{m['flow_thresh_n']:g}" if m.get("flow_throttle") else "-"
...
def main():
    ...
    ckpt_path = RESULTS / "flow_throttle_sweep_CKPT.jsonl"
    ...
    print(f"\nFLOW -- aggressor-flow throttle P&L sweep (winner frozen "
    ...
    out = RESULTS / f"flow_throttle_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"flow_throttle_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"flow_throttle_sweep_PERNAME_{stamp}.csv"
