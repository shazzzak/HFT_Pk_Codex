...
# ---- CLIP-SIZE (CAPACITY) SWEEP ----
# Does the edge scale with quote size? Clip = MULT x trailing-median trade size.
# All configs run the current production OBI throttle; ONLY the clip multiplier
# varies. (max_inv/soft_inv are 10x/3x clip in build_micro_params, so the caps
# scale proportionally too -- this is "scale the whole strategy up", the natural
# capacity test.) The clip is applied in _process via CLIP_MULTS[thr].
#   3x = current production (thr=0, the control). 2x tighter; 4/5/6x bigger.
...
CLIP_MULTS = [3.0, 2.0, 4.0, 5.0, 6.0]      # thr=0 -> 3x = current (control)
THROTTLE_MODES = [dict(_OBI) for _ in CLIP_MULTS]   # same throttle; clip varies
THROTTLE_LABELS = [f"clip{m:g}x" for m in CLIP_MULTS]
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
# threshold column: the clip multiplier for this config.
def _cfg_thr(thr):
    return f"{CLIP_MULTS[thr]:g}x"
...
def _process(args):
    ...
    if dsets is None:
        return None
    # clip
    clip = max(1, int(round(CLIP_MULTS[thr] * med)))   # CLIP-SIZE SWEEP axis
...
def main():
    ...
    ckpt_path = RESULTS / "clip_size_sweep_CKPT.jsonl"
    ...
    print(f"\nCLIP-SIZE -- capacity sweep (does the edge scale with quote size?) (winner frozen "
    ...
    if off_key is not None:
        ...
        print("\n=== control(clip3x=current) - config (PAIRED by day): positive => current wins ===")
    ...
    out = RESULTS / f"clip_size_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"clip_size_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"clip_size_sweep_PERNAME_{stamp}.csv"
