...
def _process(args):
    ...
    date, sym, exit_ticks, obi_def, use_micro, tol, ofi_win, ofi_th, micro_lam = args
    # calibration
...
def main():
            ...
            for um in MICRO_MODES:
                for tl in TOL_MODES:
                    for ofw in OFI_MODES:
                        # OFI-on configs: single thresh set, mlam=None (baseline)
                        if ofw is not None:
                            for oth in OFI_THRESH:
                                configs.append((et, od, um, tl, ofw, oth, None))
                        else:
                            # OFI-OFF config: sweep the defensive lean lambda.
                            # dummy thresh (OFI off), one row per lambda value.
    ...
    agg = {(et, od, um, tl, ofw, oth, mlam): {b: {"capture": 0.0, "markout": 0.0, "fee": 0.0,
                                  "liq_cap": 0.0, "liq_mko": 0.0,
    ...
    daily_mko = {(et, od, um, tl, ofw, oth, mlam): {} for (et, od, um, tl, ofw, oth, mlam) in configs}
    ...
    daily_net = {(et, od, um, tl, ofw, oth, mlam): {} for (et, od, um, tl, ofw, oth, mlam) in configs}
    ...
    daily_bkt = {(et, od, um, tl, ofw, oth, mlam): {} for (et, od, um, tl, ofw, oth, mlam) in configs}
    ...
    name_bkt = {(et, od, um, tl, ofw, oth, mlam): {} for (et, od, um, tl, ofw, oth, mlam) in configs}
    # per-config RAW engine P&L accumulator (the reconciliation anchor)
    engine_pnl_agg = {(et, od, um, tl, ofw, oth, mlam): 0.0
                      for (et, od, um, tl, ofw, oth, mlam) in configs}
    ...
    recon_gap_agg = {(et, od, um, tl, ofw, oth, mlam): 0.0
                     for (et, od, um, tl, ofw, oth, mlam) in configs}
    # run
    done = 0
    ...
    for (et, od, um, tl, ofw, oth, mlam) in configs:
        a = agg[(et, od, um, tl, ofw, oth, mlam)]
    ...
    off_key = next(((et, od, um, tl, ofw, oth, mlam)
                    for (et, od, um, tl, ofw, oth, mlam) in configs if ofw is None), None)
    ...
    print(f"{'ofi_window':>14s} {'thresh':>7s} {'n_days':>7s} "
          f"{'mean_bps':>9s} {'se':>7s}")
    for (et, od, um, tl, ofw, oth, mlam) in configs:
        ser = _day_bps_series((et, od, um, tl, ofw, oth, mlam))
    ...
    if off_key is not None:
        ...
        print("\n=== OFF - OFI (PAIRED by day): positive => OFF wins ===")
        print(f"{'ofi_window':>14s} {'thresh':>7s} {'n_pair':>7s} "
              f"{'d_bps':>8s} {'se':>7s} {'t':>7s}")
        for (et, od, um, tl, ofw, oth, mlam) in configs:
            # skip the OFF row itself
            if ofw is None:
                continue
            ser = _day_bps_series((et, od, um, tl, ofw, oth, mlam))
            ...
            wlab = f"min{ofw[0]}ev{ofw[1]:.0f}s"
            print(f"{wlab:>14s} {oth:>7.2f} {m:>7d} {dm:>8.3f} {dse:>7.3f} "
                  f"{tstat:>7.2f}")
        # reading guide
        print("  (|t| >~ 2 => OFF's lead over that OFI variant is beyond "
              "day-to-day noise)")
    ...
    for (et, od, um, tl, ofw, oth, mlam) in configs:
        ...
        eng = engine_pnl_agg[(et, od, um, tl, ofw, oth, mlam)]
    ...
    for (et, od, um, tl, ofw, oth, mlam) in configs:
        ...
        dm = pd.Series(daily_mko[(et, od, um, tl, ofw, oth, mlam)]).sort_index().to_numpy()
        if len(dm) >= 20:
    ...
    for (et, od, um, tl, ofw, oth, mlam) in configs:
        ...
        rows.append({"exit_ticks": et, "obi_defensive": od, "use_microprice": um,
                     ...
                     "net_pkr": net,
                     "engine_pnl_pkr": engine_pnl_agg[(et, od, um, tl, ofw, oth, mlam)],
