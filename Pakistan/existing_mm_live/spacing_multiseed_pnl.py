...
# Freeze the chosen symbols, dates and seeds before examining profit.
def select_jobs(manifest, seeds, period):
    # Preserve the original four-stock comparison universe.
    symbols = ('NRL', 'MLCF', 'NPL', 'NCPL')
    # Require multiple distinct nonnegative seeds.
    if len(seeds) < 3 or len(set(seeds)) != len(seeds) or any(type(seed) is not int or seed < 0 for seed in seeds):
        # Refuse duplicate random streams masquerading as additional tests.
        raise ValueError('Provide at least three distinct nonnegative integer seeds')
    # Select only explicitly declared production jobs for the pilot universe.
    selected = [j for j in manifest['jobs'] if j['symbol'] in symbols]
    # Derive dates from the actual selected jobs.
    dates = sorted({j['date'] for j in selected})
    # Require enough dates for this expansion.
    if len(dates) < 20:
        # Prevent an accidental rerun of the three-date pilot.
        raise ValueError('At least twenty dates are required')
    # Require complete, duplicate-free stock-date coverage.
    if len(selected) != len(symbols)*len(dates) or len({(j['symbol'],j['date']) for j in selected}) != len(selected):
        # A missing or repeated cell invalidates the declared experiment.
        raise ValueError('Manifest must contain each stock exactly once on every date')
    # Validate date syntax before comparisons.
    for date in dates:
        # ISO dates must be complete calendar dates.
        if datetime.strptime(date, '%Y-%m-%d').strftime('%Y-%m-%d') != date:
            # Reject ambiguous or noncanonical dates.
            raise ValueError('Dates must be YYYY-MM-DD')
    # Preserve the existing June scope for the expanded comparison.
    if period == 'comparison' and (len(dates) != 20 or dates[0] != '2026-06-01' or dates[-1] != '2026-06-30'):
        # Require a separately declared validation run for later dates.
        raise ValueError('Comparison requires the frozen twenty-date June 2026 manifest')
    # Do not relabel previously researched history as untouched validation.
    if period == 'validation' and dates[0] <= '2026-06-30':
        # Existing research already included this history.
        raise ValueError('Validation dates must all be after 2026-06-30')
    # Validate production sizing and the fixed quote-skew policy.
    for job in selected:
        # Read the immutable strategy settings carried by this date.
        params = job['params']
        # Preserve positive integer production clips and the tested queue-skew rule.
        if type(job['clip']) is not int or job['clip'] <= 0 or params['size'] != job['clip'] or params.get('queue_skew_ticks') != 2.0 or params.get('queue_skew_thresh') != 0.15 or params.get('queue_skew_stairs') or params.get('queue_skew_bps', 0) != 0:
            # Different policy settings would confound this experiment.
            raise ValueError('Expected valid production clip and unchanged QT_2t@15 settings')
        # Do not carry a June absolute session into a later validation day.
        segments = params.get('session_segments', [])
        # Require date-appropriate session calibration in every provided manifest.
        if not segments or any(datetime.fromtimestamp(start/1000, tz=timezone.utc).strftime('%Y-%m-%d') != job['date'] or end <= start for start,end in segments):
            # A later period needs its own properly prepared frozen settings.
            raise ValueError('Manifest session segments do not match the trading date')
    # Replicate each whole stock-day across the declared latency seeds.
    return [dict(copy.deepcopy(job), seed=seed) for job in selected for seed in seeds], dates
...
def main():
    ...
    parser.add_argument('--period', choices=('comparison','validation'), default='comparison')
        ...
        with context.Pool(args.workers, initializer=initialize, initargs=(progress,)) as pool:
            ...
            while pending:
                ...
                if time.monotonic()-last_print >= 15:
                    ...
                    eta = f'~{elapsed*(planned-done)/done:.1f}m' if done > 0 else 'estimating (loading)'
    ...
    summary = dict(completed=clean, planned=planned, period=args.period, seeds=args.seeds, validation_completed=(args.period == "validation" and clean), completed_cells=len(rows), failed=sum(not r['passed'] for r in rows), changed_inputs=changed, live_approved=False)
    ...
    if clean:
        ...
        axis = daily.plot.bar(figsize=(14, 5), ylabel='Net profit (PKR)', title=f'{args.period}: daily profit averaged across latency seeds')
