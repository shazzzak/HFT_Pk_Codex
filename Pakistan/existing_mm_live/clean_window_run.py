...
def main():
    ...
    source={str(path):digest(path) for path in sorted(Path(__file__).parent.glob('*.py'))}
    # Keep resume compatibility tied to source, scope and sizing evidence.
    identity=dict(assignment_sha256=digest(args.assignment),manifest_sha256=digest(args.manifest),source=source,jobs=[(j['symbol'],j['date']) for j in jobs],experiment='retrospective_clean_windows_v1',seed=0,arms=list(ARMS),min_window_ms=60000,cancel_buffer_ms=5000,exit_send_buffer_ms=2000)
    ...
    if source!={str(path):digest(path) for path in sorted(Path(__file__).parent.glob('*.py'))}:
