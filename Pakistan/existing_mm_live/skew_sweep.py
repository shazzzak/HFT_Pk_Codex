...
# ---- SIZE / QUEUE / INVENTORY-TAPER P&L SWEEP ----
# Three untested skew families, all INCREMENTAL over the OBI throttle. None of
# them spends spread capture (the mechanism that killed every price-skew test):
# size skew and the inventory taper change QUANTITY; queue skew changes QUEUE
# POSITION. Price and fair value are untouched throughout.
#   OBI (thr=0)   = current production = control.
#   SIZE_x        = favorable-side clip boosted x (OBI-signal driven).
#   QUEUE_n       = favorable side n tick(s) tighter, exposed side n back.
#   POV_p         = unwind_pov changed ONLY (taper off) -- isolates the effect of
#                   a more realistic participation rate on the EOD unwind trigger.
#   TAPER_*       = inventory-driven size taper on POV capacity, at the lower POV.
#                   'add' = shrink the adding side; 'both' = also boost the
#                   reducing side. k = sensitivity exponent.
...
# favorable-side size multipliers (OBI-driven size skew)
_SIZE_MULTS = [1.5, 2.0]
# queue-skew tick shifts
_QUEUE_TICKS = [1.0, 2.0]
# participation rates to test (production is 0.10; 0.025/0.05 are realistic)
_POVS = [0.05, 0.025]
...
THROTTLE_LABELS = ["OBI"]
# 1) OBI-signal size skew
for _m in _SIZE_MULTS:
    THROTTLE_MODES.append(dict(_OBI, size_boost_mult=_m, size_boost_thresh=0.15))
    THROTTLE_LABELS.append(f"SIZE_{_m:g}x")
# 2) queue / aggressiveness skew
for _q in _QUEUE_TICKS:
    THROTTLE_MODES.append(dict(_OBI, queue_skew_ticks=_q, queue_skew_thresh=0.15))
    THROTTLE_LABELS.append(f"QUEUE_{_q:g}t")
# 3) POV alone (taper OFF) -- isolates the unwind-trigger effect of a lower POV
for _p in _POVS:
    THROTTLE_MODES.append(dict(_OBI, unwind_pov=_p))
    THROTTLE_LABELS.append(f"POV_{_p*100:g}pct")
# 4) inventory taper at the lower POV, add-side only and both-sides, k=1 and 2
for _p in _POVS:
    for _k in _TAPER_K:
        for _both in (False, True):
            THROTTLE_MODES.append(dict(_OBI, unwind_pov=_p, enable_inv_taper=True,
...
def _cfg_thr(thr):
    ...
    parts = []
    if m.get("size_boost_mult", 1.0) != 1.0:
        parts.append(f"{m['size_boost_mult']:g}x")
    if m.get("queue_skew_ticks", 0.0) != 0.0:
        parts.append(f"{m['queue_skew_ticks']:g}t")
    if m.get("unwind_pov", 0.10) != 0.10:
        parts.append(f"pov{m['unwind_pov']*100:g}")
    if m.get("enable_inv_taper"):
        parts.append(f"k{m['inv_taper_k']:g}")
...
def main():
    ...
    ckpt_path = RESULTS / "skew_sweep_CKPT.jsonl"
    ...
    print(f"\nSIZE-SKEW + QUEUE-SKEW -- quantity and queue-position asymmetry (winner frozen "
    ...
    out = RESULTS / f"skew_sweep_{stamp}.csv"
