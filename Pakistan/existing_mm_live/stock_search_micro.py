...
class MicrostructureMM:
    ...
    def quotes(self, bb, bq, ba, aq, pos, depth=None):
        ...
        imb = bq / (bq + aq)
        # Replace only the throttle, defensive and queue signals in this research copy.
        decision_imb = self.search_signal(bb, ba, imb) if hasattr(self, "search_signal") else imb
        ...
        buy_trig = ((self.obi_throttle
                     and (0.5 - decision_imb) > self.obi_throttle_thresh)
        ...
        sell_trig = ((self.obi_throttle
                      and (decision_imb - 0.5) > self.obi_throttle_thresh)
                     or (self.ofi_throttle and ofi_sig is not None
                         and ofi_sig > self.ofi_throttle_thresh)
                     or qdr_sell_trig
                     or flow_sell_trig)
        ...
        if self.queue_skew_stairs or self.queue_skew_ticks != 0.0 \
            ...
            if (decision_imb - 0.5) > self.queue_skew_thresh:
            ...
            elif (0.5 - decision_imb) > self.queue_skew_thresh:
        ...
        if (not trig["kill_buy"]) and pos < eff_max \
            ...
            if self.obi_defensive and (0.5 - decision_imb) > self.obi_defensive_thresh:
        ...
        if self._cap_ask_at_position and pos <= 0:
            ...
            if self.obi_defensive and (decision_imb - 0.5) > self.obi_defensive_thresh:
