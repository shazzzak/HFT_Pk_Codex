...
class WindowEngine(Engine):
    ...
    def __init__(self,strategy,cfg,window,adds):
        ...
        self.book=WindowBook(window['checkpoint'],adds)
    ...
    def replay(self,events,window,progress=None):
        ...
        return dict(net_pkr=self.cash,fills=len(self.fills),max_inventory=self.max_inventory,drawdown_pkr=self.drawdown,buckets=buckets,capacity_reductions=self.capacity_reductions)
