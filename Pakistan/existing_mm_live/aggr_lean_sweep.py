...
# ---- AGGRESSION-LEAN P&L SWEEP ----
# Shift fair value toward recent volume-weighted aggression imbalance (1s window),
# gated to OBI-calm rows. OFFENSIVE lean (follow the continuation) -- tested
# INCREMENTAL over OBI. Sweep the lean strength k.
#   OBI (thr=0) = current production = control.
#   AL_k0.25 / 0.5 / 1.0 = OBI + aggression-lean at that strength.
...
# lean strengths to sweep (x imbalance x spread)
_AL_K = [0.25, 0.5, 1.0]
# thr=0 OBI control; then OBI + aggression-lean at each k
THROTTLE_MODES = [dict(_OBI)]
THROTTLE_LABELS = ["OBI"]
for _k in _AL_K:
    THROTTLE_MODES.append(dict(_OBI, enable_aggr_lean=True, aggr_lean_k=_k,
                               aggr_lean_win_ms=1000.0, aggr_lean_obi_calm=0.30))
    THROTTLE_LABELS.append(f"OBI+AL_k{_k}")
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
def _cfg_thr(thr):
    m = THROTTLE_MODES[thr]
    return f"k{m['aggr_lean_k']:g}" if m.get("enable_aggr_lean") else "-"
...
def main():
    ...
    ckpt_path = RESULTS / "aggr_lean_sweep_CKPT.jsonl"
    ...
    print(f"\nAGGRESSION-LEAN -- fair-value lean on aggression imbalance (winner frozen "
    ...
    out = RESULTS / f"aggr_lean_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"aggr_lean_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"aggr_lean_sweep_PERNAME_{stamp}.csv"
