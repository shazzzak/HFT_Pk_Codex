...
# Compute top-three price-distance bid share, falling back to the unchanged signal.
def distance_share(bids, asks, bb, ba, tick, fallback):
    # Require three known levels on both sides and a valid tick.
    if len(bids) < 3 or len(asks) < 3 or not math.isfinite(tick) or tick <= 0:
    ...
    for levels, touch, sign in ((bids[:3], bb, -1), (asks[:3], ba, 1)):
        # Start the side's weighted sum.
        total = 0.0
        # Remember the prior level to reject malformed ordering.
        previous = -1.0
        # Examine exactly three quoted prices.
        for price, qty in levels:
            ...
            if not all(math.isfinite(x) for x in (price, qty, distance)) or qty <= 0 or distance <= previous or abs(distance-round(distance)) > 1e-5:
                ...
                return fallback, False
            # Discount quantities by actual price distance, with fixed k=0.1.
            total += qty * math.exp(-0.1 * max(0.0, distance))
...
def cell(task):
    ...
    try:
        ...
        for arm_index, arm in enumerate(('baseline', 'copy_control', 'distance')):
            ...
            counts = dict(calls=0, valid=0, fallback=0, changed_skew=0)
            ...
            def signal(bb, ba, original):
                ...
                bids, asks = engine.book.ranked_depth(3, include_deep=False)
                # Calculate the fixed candidate signal.
                value, valid = distance_share(bids, asks, bb, ba, strategy.tick, original)
            ...
            def stream():
                ...
                for index, event in enumerate(events):
                    ...
                    if index % 1000 == 0 and time.monotonic()-last_update[0] >= 1:
                        # Store a three-arm completion fraction.
                        PROGRESS[key] = (arm, (arm_index + index/len(events))/3)
        ...
        row = dict(job=job, passed=True, decoded_input_sha256=fingerprint, arms=results, delta_pkr=results['distance']['net_pkr']-results['baseline']['net_pkr'])
...
def main():
    ...
    if clean:
        ...
        table = pd.DataFrame([dict(symbol=r['job']['symbol'], date=r['job']['date'], arm=arm, **{k: value[k] for k in ('net_pkr', 'fills', 'shares', 'max_abs_inventory', 'max_drawdown_pkr')}, liquidation_clean=value['eod']['liquidation_clean'], residual_shares=value['eod']['unfilled_sh']) for r in rows for arm, value in r['arms'].items() if arm != 'copy_control'])
        ...
        daily['difference'] = daily.distance-daily.baseline
        ...
        summary.update(baseline_net_pkr=float(daily.baseline.sum()), distance_net_pkr=float(daily.distance.sum()), difference_pkr=float(daily.difference.sum()), winning_dates=int((daily.difference > 0).sum()), tested_dates=len(daily), changed_skew=sum(r['arms']['distance']['signal']['changed_skew'] for r in rows))
        ...
        axis = daily[['baseline', 'distance']].plot.bar(figsize=(9, 4), ylabel='Net profit (PKR)', title='Same dates, sizes, fees and latency settings')
