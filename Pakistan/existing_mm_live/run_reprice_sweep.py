...
# ---- RUN-REPRICE P&L SWEEP ----
# After N consecutive same-side aggressor ORDERS, push the EXPOSED side away from
# the touch by a PER-TICKER tick count (derived from measured post-run markout),
# so a run lets us sell higher / buy cheaper. Tested INCREMENTAL over OBI.
#   OBI (thr=0) = current production = control.
#   N2/N3/N4 = run-reprice on top of OBI at trigger run length 2/3/4.
# The per-name reprice ticks come from REPRICE_TICKS below (measured 5s markout at
# run>=3 -> price -> ticks, rounded up to >=1; the 5 high-priced names get >1).
...
# per-name reprice tick count (from the bps->tick table; default 1)
REPRICE_TICKS = {"PIOC":5, "NRL":4, "ATRL":4, "SAZEW":4, "HBL":2}
# the 5 names where the measured move exceeds 1 tick (the real candidates)
CANDIDATES = ["PIOC", "NRL", "ATRL", "SAZEW", "HBL"]
# trigger run lengths to sweep
_RR_N = [2, 3, 4]
# thr=0 OBI control; then OBI + run-reprice at each N
THROTTLE_MODES = [dict(_OBI)]
THROTTLE_LABELS = ["OBI"]
for _n in _RR_N:
    THROTTLE_MODES.append(dict(_OBI, enable_run_reprice=True, run_reprice_n=_n))
    THROTTLE_LABELS.append(f"OBI+RR_N{_n}")
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
def _cfg_thr(thr):
    m = THROTTLE_MODES[thr]
    return f"N{m['run_reprice_n']}" if m.get("enable_run_reprice") else "-"
...
def _process(args):
    ...
    # RUN-REPRICE: set the per-ticker reprice tick count when the reprice is on
    if params.get("enable_run_reprice"):
        # measured per-name ticks (default 1 for names not in the table)
        params["run_reprice_ticks"] = REPRICE_TICKS.get(sym, 1)
    # run
...
def main():
    ...
    ckpt_path = RESULTS / "run_reprice_sweep_CKPT.jsonl"
    ...
    print(f"\nRUN-REPRICE -- push exposed side on same-side runs (winner frozen "
    ...
    out = RESULTS / f"run_reprice_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"run_reprice_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"run_reprice_sweep_PERNAME_{stamp}.csv"
