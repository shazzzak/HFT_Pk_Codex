...
import argparse
# Verify the actual constructed settings in every replay.
from assigned_spacing_config import validate_strategy, COMMON
...
ARMS = {'baseline': None, 'distance_3': (3, 0.1), 'distance_4': (4, 0.1), 'distance_5': (5, 0.1)}
...
def cell(task):
    ...
    try:
        ...
        # Replay the assigned baseline and the three fixed weighted depths.
        for arm_index, arm in enumerate(ARMS):
            # Build a fresh strategy with identical frozen production settings.
            # The replacement share cannot affect quotes when both queue-skew modes are off.
            if arm != 'baseline' and job['params']['queue_skew_ticks'] == 0 and job['params']['queue_skew_bps'] == 0:
                # Reuse only this same job's newly computed corrected baseline.
                results[arm] = copy.deepcopy(results['baseline'])
                # Explain the exact reuse instead of pretending it was replayed.
                results[arm]['reused_zero_skew_baseline'] = True
                # Preserve the full four-version evidence.
                save(folder / 'arms.json', results)
                # Skip a provably identical historical simulation.
                continue
            # Instantiate the assigned baseline or queue-hook candidate.
            strategy = (Original if arm == 'baseline' else Candidate)(session_ms=session, **copy.deepcopy(job['params']))
            # Abort this cell before replay if any requested setting is wrong.
            validate_strategy(strategy, job)
            ...
            def signal(bb, ba, original):
                ...
                return value
            # Attach the replacement share only to the three weighted candidates.
            if spec is not None:
...
def select_jobs(manifest, seeds, period):
    # Freeze the user's one-seed choice rather than quietly expanding compute.
    if seeds != [0] or any(type(seed) is not int for seed in seeds):
        # Additional seeds require a separately declared experiment.
        raise ValueError('This experiment requires seed 0 only')
    # Refuse an old uniform manifest or missing explicit controls.
    if manifest.get('experiment') != 'assigned_spacing_v1' or manifest.get('strategy_mode') != 'assigned' or manifest.get('common_settings') != COMMON:
        # Prevent the previous baseline mistake before starting a pool.
        raise ValueError('Expected the explicit assigned-spacing manifest')
    # Require the user's revised all-stock scope with no inherited DROP exclusions.
    if manifest.get('all_stocks_trade') is not True or manifest['excluded']:
        # Fail before a full run if any stock was silently removed.
        raise ValueError('This experiment must include all stocks without exclusions')
...
def compact(row):
    ...
    result['job'] = {k:row['job'][k] for k in ('symbol','date','seed','assignment','original_assignment')}
...
def main():
    ...
    # Use the user-requested single random stream.
    ...
    save(output / 'frozen.json', dict(jobs=jobs, hashes=hashes, cfg=R.CFG, latency_seeds=args.seeds, period=args.period, latency_defaults=vars(LatencyModel(seed=R.LATENCY_SEED)), arms=ARMS, policy='Common requested configuration and acquisition cap are ON in every arm. Only queue-skew bid share changes: exp(-k * own-touch distance in ticks) at declared depth; insufficient/invalid depth falls back to L1; original thresholds unchanged', use_cfo=True, cross_on_arrival=True, market='REG', scope='Assigned portfolio, October-June; September sizing warmup; seed 0; requested shared controls including acquisition cap ON; retrospective calibration; no independent validation or live approval'))
    ...
    if clean:
        ...
        # Map the original portfolio classification separately from the trading assignment.
        labels = {r['job']['symbol']:(r['job']['original_assignment'],r['job']['assignment']) for r in rows}
        # Expose which formerly excluded stocks were reintroduced.
        table['original_assignment'] = table.symbol.map(lambda symbol:labels[symbol][0])
        # Preserve the fixed baseline label used for every weighted version.
        table['assignment'] = table.symbol.map(lambda symbol:labels[symbol][1])
        table.to_csv(output / 'comparison.csv', index=False)
        # Aggregate daily profits across the assigned stocks.
        ...
        # Aggregate assigned stocks only after averaging seeds within each stock-day.
        ...
        # Separate revived DROP stocks from the originally traded portfolio.
        averaged['original_group'] = averaged.symbol.map(lambda symbol:'formerly_DROP' if labels[symbol][0]=='DROP' else 'originally_traded')
        # Show whether weighted signals improve the previously excluded names.
        averaged.groupby(['original_group','arm']).net_pkr.sum().unstack().to_csv(output/'profit_by_original_group.csv')
        ...
        # Report averages without counting repeated seeds as additional dates.
        summary['profit_averages'] = {arm: dict(total_pkr=float(daily[arm].sum()), average_trading_day_pkr=float(daily[arm].mean()), average_month_pkr=float(monthly[arm].mean())) for arm in daily.columns}
