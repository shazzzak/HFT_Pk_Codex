...
# ---- AGE-AWARE EXIT P&L SWEEP ----
# FIFO showed round trips held >15min turn toxic (realized <0). Test: force-exit
# aged positions (engage the exit skew once held past a threshold) -- does it
# recover the long-hold drag, or just realize losses sooner? All configs run the
# current production OBI throttle; only the age cutoff varies.
#   OBI (thr=0) = current production, NO age exit = the control.
#   age10/15/20 = engage the exit skew once a position is held that many minutes.
_OBI = dict(obi_throttle=True, ofi_throttle=False, obi_throttle_thresh=0.15,
            ...
            enable_pov_cap=False, flow_throttle=False, enable_age_exit=False)
_AGE_MIN = [10.0, 15.0, 20.0]      # minutes-held cutoffs to sweep
THROTTLE_MODES = [dict(_OBI)]
THROTTLE_LABELS = ["OBI"]
for _a in _AGE_MIN:
    THROTTLE_MODES.append(dict(_OBI, enable_age_exit=True, age_exit_ms=_a * 60000.0))
    THROTTLE_LABELS.append(f"age{_a:g}m")
def _cfg_lab(thr):
    return THROTTLE_LABELS[thr]
def _cfg_thr(thr):
    m = THROTTLE_MODES[thr]
    return f"{m['age_exit_ms']/60000:g}m" if m.get("enable_age_exit") else "-"
...
def main():
    ...
    ckpt_path = RESULTS / "age_exit_sweep_CKPT.jsonl"
    ...
    print(f"\nAGE-EXIT -- force-exit aged positions P&L sweep (winner frozen "
    ...
    if off_key is not None:
        ...
        print("\n=== control(OBI, no age-exit) - config (PAIRED by day): positive => current wins ===")
    ...
    out = RESULTS / f"age_exit_sweep_{stamp}.csv"
    ...
    dout = RESULTS / f"age_exit_sweep_DAILY_{stamp}.csv"
    ...
    nout = RESULTS / f"age_exit_sweep_PERNAME_{stamp}.csv"
