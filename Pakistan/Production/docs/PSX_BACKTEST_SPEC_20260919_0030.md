...
own output file. PART IV (§38) was written after `wobi_headtohead.py`, which
overturns this document's largest negative conclusion: the decay-weighted OBI
work does exist. PART V (§39) reports the 113-name re-run under the corrected
engine — which moves every P&L level in §20 and leaves one 25% shift
unexplained.** It closes three §34 gaps and **corrects two statements made
...
single-page index of every number in the document with its source. §37 is the
correction register — every claim this document got wrong, with the evidence.
§38 is the decay-weighted OBI head-to-head, and §39 the corrected-engine
re-run. **§20's PKR levels and §39's PKR levels come from two different engines
and must not be quoted in the same table until §39.4 is resolved.**
...
**THE DECAY WORK EXISTS — this subsection's original conclusion was wrong and
is corrected in §38.** `wobi_headtohead.py` runs plain L1 against two
decay-weighted variants (`L1 + 0.3·L2` and `L1 + 0.5·L2`) as a paired P&L
head-to-head, on the back of a screen called `obi_decay_gate`. So the right
statement is narrower than the one this subsection used to make:

- **The files listed above are equal-weight** — that part stands, and the OFI
  comment confirms it was deliberate *for the multi-level OFI*.
- **The weighting lives in `micro_mm` behind `weighted_obi` / `wobi_depth` /
  `wobi_decay`**, not in the feature store, and not in any copy of `micro_mm.py`
  supplied to this document (§38.7).
- **`build_feature_store.py` genuinely does not contain one** (§37.2) — the only
  depth weighting there is the level-1 microprice. That finding is unaffected;
  it was the *generalisation* from it that was wrong.

`obi_decay_gate` itself is still unread and is the one file that would close the
screen half of this. `expand_feature_store.py` and `probe_conditional_markout.py`
remain unread for their own reasons (§34).
...
|---|---|---|
| 1 | ~~The weighted / decay-sensitivity OBI work does not exist~~ — **WRONG, and closed as an existence question in §38.** It exists: `obi_decay_gate` (screen) and `wobi_headtohead.py` (paired P&L head-to-head, `L1 + r·L2` at r ∈ {0.3, 0.5}) | — |
| 1a | **`obi_decay_gate`** — the screen behind it. Its two headline numbers (+0.023 incremental partial-Spearman over L1, +0.003 standalone) are quoted from `wobi_headtohead.py`'s header, not verified | the file, and its output |
| 1d | **No result of the decay head-to-head exists anywhere.** And `weighted_obi` is absent from every `micro_mm.py` copy supplied, so the run may not yet be possible (§38.7) | check the kwarg, then run it — **at the full panel, not 20 days** (§38.6) |
...
23. **"I did not find it" is not "it is not there."** This document twice
    concluded the decay-weighted OBI work did not exist, from a file list that
    did not contain it. Both statements were hedged and both still misled.
    **Report the search, not the conclusion** — name what was looked at, and let
    the absence stay an absence. (§38.8)
24. **Match the sample to the effect before running, not after.** A 20-day
    paired test on this book resolves nothing smaller than **0.87 bps/day**; the
    whole QT_2t edge is 1.30. A negative result from an underpowered run is
    indistinguishable from a wiring bug, which is the exact failure the run's own
    guards exist to prevent. **Print the minimum detectable effect beside every
    "no difference" verdict.** (§38.6)
25. **Test that a guard fires.** `wobi_headtohead.py` plants a deliberately
    broken arm in its self-test and asserts the validator rejects it. Almost
    nothing else here does. A guard never seen to fire is `PriceBandCheck`
    waiting to happen (lesson 12). (§38.3)
26. **Recompute a derived number before repeating it.** "91.7% of the per-name
...
4. The **`leanband@0.25` and `leanband@0.40` arms** of the extreme-OBI sweep —
   ...
   header prose.)
5. ~~The **113-name re-run under the corrected engine**~~ — **DONE, §39.**
   66,783 cells, 600 minutes, anchors 0.00. The assignment moves 6 names of 113.
6. The **decay-weighted OBI head-to-head** (§38). The script exists, self-tests
   clean, and has no recorded output. As configured (20 days) it could only
   resolve an effect of **0.87 bps/day or larger**, so a "FLAT" verdict from it
   would carry almost no information — run it at the full panel.
...
**In the files covered by this section there is no per-level decay** — that
remains true of `build_feature_store.py`, `extreme_obi_sweep.py`, `Book.obi`,
the multi-level OFI and every copy of `micro_mm.py` supplied here.

**But the sentence that used to stand here — "no decay sensitivity sweep in any
file read for this document" — generalised from that file list to the project,
and it was wrong.** `wobi_headtohead.py` (§38) is the decay work: two decay
ratios at depth 2, judged on money rather than correlation. The correction, and
why the hedge in the original wording did not save it, is §38.8.
...
source files. **Twelve statements were wrong.** They are listed here, with the
...
| **12** | **The decay-weighted OBI work does not exist in this codebase** (§27.2, §37.2, §34 row 1) | **It exists** — `obi_decay_gate` and `wobi_headtohead.py`, §38. This is the largest of the twelve | **Generalised from "not in the files I read" to "not in the project"**, and the hedge in the wording did not stop the emphasis carrying the wrong claim |
...
**#12 is the largest, and it is its own failure mode.** Every sentence stating
it was hedged — "in any file I have read", with the unread candidates listed by
name — and it still misled, because the *emphasis* asserted more than the
*evidence* supported. **An absence of evidence written with the confidence of
evidence of absence is a claim.** The fix is not more hedging; it is to report
what was searched and stop.

**The deepest of the rest is #2**, and it is worth naming on its own: faced with a count
...
### 36.16 Part IV additions

| quantity | value | basis | § |
|---|---|---|---|
| decay arms | `L1` (control), `L1 + 0.3·L2`, `L1 + 0.5·L2` | code | 38.2 |
| `ofi_depth_levels` required by a weighted arm | **≥ `wobi_depth`**, else the signal collapses to L1 | code | 38.3 |
| where the engine supplies depth | `mm_backtest.py` **2066–2071** (the file's header cites 1209–1210; stale) | code | 38.3 |
| screen result, incremental partial-Spearman over L1 | **+0.023** (`w_d2_r0.5`, the largest) | **quoted from the header; `obi_decay_gate` not read** | 38.1 |
| screen result, standalone vs L1 | **+0.003** | same | 38.1 |
| head-to-head baseline | `obi_throttle=True`, `ofi_throttle=False`, `obi_throttle_thresh=0.15`, `throttle_frac=0.5`, `throttle_hold_ms=300.0` | code | 38.4 |
| run shape | watchlist universe × **20 days** × 3 arms, 9 workers, `TRAIL_DAYS=10`, `CLIP_MULT=3.0` | code | 38.4 |
| pre-registered verdict rule | WIN if `m>0 ∧ t>+2`; LOSE if `m<0 ∧ t<−2`; else **FLAT → keep L1** | code | 38.5 |
| implied daily paired sd on this book | **1.94 bps** (from +1.3036 bps/day at t=+9.42 over 197 dates) | derived | 38.6 |
| minimum detectable effect at 20 days | **0.87 bps/day** at \|t\|=2 | derived | 38.6 |
| …at 60 / 100 / 197 days | 0.50 / 0.39 / 0.28 bps/day | derived | 38.6 |
| days needed to resolve 0.30 bps/day | **168** | derived | 38.6 |
| `weighted_obi` in any supplied `micro_mm.py` | **absent** (3 copies checked, 0 hits) | verified | 38.7 |
| recorded result of the head-to-head | **none** | — | 38.7 |

---

# PART IV — ADDED 2026-09-18 (fourth pass)

One file: **`wobi_headtohead.py`**. It overturns the largest negative conclusion
in this document.

**§27.2, §37.2 and §34 row 1 all say the decay-weighted OBI work does not exist
in this codebase. That is now wrong. It exists.** There is a screen
(`obi_decay_gate`), a weighting implementation in the strategy
(`weighted_obi` / `wobi_depth` / `wobi_decay`), and a money head-to-head against
plain L1. The correction is #12 in the register at §37.4.

What has **not** happened is a recorded result, and — on every copy of the
engine in hand — the strategy edits the head-to-head depends on have not been
made. §38.7 states exactly what is and is not established.

---

## 38. DECAY-WEIGHTED OBI — `wobi_headtohead.py`

### 38.1 What it is, and the project rule behind it

Not another correlation study. A **paired P&L head-to-head**: run the production
strategy across the universe twice — once on plain level-1 imbalance, once with
the decay-weighted signal swapped in — and compare net bps per day.

The reason is stated at the top of the file, and it is the right reason:

> *"The `obi_decay_gate` SCREEN said `w_d2_r0.5` has the largest INCREMENTAL
> partial-Spearman over L1 (**+0.023**) but ~zero standalone advantage
> (`vs_L1` **+0.003**). Per the project rule **"do not close on the correlation
> metric alone,"** this settles it in money."*

That is the correct order of operations and it is the same discipline as the
lead-lag screen's economic gate (§30) and the cheap-tick finding (§20.1): **a
signal that correlates is not a signal that pays.** Two numbers carry the whole
motivation, and they disagree in an informative way — a small *incremental*
contribution on top of L1, and essentially nothing standalone. That pattern says
"if it helps at all, it helps only where L1 is uninformative", which is exactly
the kind of claim only money can settle.

**Provenance caveat: both numbers are quoted from this file's header. I have not
read `obi_decay_gate` itself.** It goes in §34.

### 38.2 The weighting, stated exactly as far as the file determines it

| arm | `weighted_obi` | `wobi_depth` | `wobi_decay` | `ofi_depth_levels` | the signal |
|---|---|---|---|---|---|
| `L1` (**control**) | `False` | — | — | 1 (default) | plain `bq/(bq+aq)` at the touch |
| `wobi_d2r0.3` | `True` | 2 | 0.3 | **2** | L1 + **0.3 ×** L2 |
| `wobi_d2r0.5` | `True` | 2 | 0.5 | **2** | L1 + **0.5 ×** L2 |

The naming is `w_d{depth}_r{ratio}`. At depth 2 the weight vector is `[1, r]`,
stated in the file's own comments as "L1 + 0.3*L2". **The form beyond depth 2 is
not determined by this file** — geometric `r^(k−1)` is the obvious reading of a
parameter called `decay`, but nothing here proves it, and `wobi_depth` is 2 in
every arm. A rebuild must read `_weighted_imb` in `micro_mm.py` for the general
case.

**This is the decay sensitivity: two decay ratios at fixed depth, judged on
money.** It is a narrower sweep than "sensitivity analysis over decay levels"
suggests — two points, one depth — but it is aimed at the only question that
matters, which is more than a wide correlation sweep would be.

### 38.3 The silent-null trap — the best engineering in the file

This is the part worth copying into anything else in the project.

**The trap.** `mm_backtest.py` supplies ranked depth to the strategy **only when
`ofi_depth_levels > 1`**, and supplies exactly that many levels:

```python
# mm_backtest.py (engine 55b239ed60d3, lines 2066-2071)
...
So a weighted arm that forgets to raise `ofi_depth_levels` receives `depth=None`,
has nothing to weight, **silently falls back to level 1 — and the entire
experiment becomes L1 against L1.** It would run to completion, produce a clean
paired t-statistic of approximately zero, and be indistinguishable from a real
negative result. That is the worst failure mode an experiment can have: it
confirms the null by construction and leaves no trace.

**The defence is three-deep, and each layer catches a different mistake:**

```python
# 1. PRE-POOL, before any expensive work: every weighted arm must ask for
#    at least as many levels as it intends to weight.
assert odl >= wd, (f"ARM '{label}' misconfigured: ofi_depth_levels={odl} < "
                   f"wobi_depth={wd}. ... would silently collapse to L1.")

# 2. PER CALL, inside the worker, after BASELINE and the arm overrides have
#    both been merged -- catches a later edit that reorders the merge.
if params.get("weighted_obi") and int(params.get("ofi_depth_levels",1)) < int(params.get("wobi_depth",1)):
    raise ValueError(f"{arm_label}: ... weighted signal would collapse to L1")

# 3. THE ENGINE ITSELF: MicrostructureMM(**params) raises TypeError if the
#    weighted_obi kwargs are absent. It will NOT silently pass.
```

**And the self-test plants a deliberately broken arm and asserts the guard
rejects it:**

```python
bad = {"bad": dict(weighted_obi=True, wobi_depth=2, ofi_depth_levels=1)}
try:    validate_arms(bad)
except AssertionError: raised = True
assert raised, "validate_arms failed to catch ofi_depth_levels < wobi_depth"
```

**Testing that a guard fires, not just that the happy path works, is rare in
this codebase and should be standard.** A guard nobody has ever seen fire is
§35 lesson 12 waiting to happen — `PriceBandCheck` was written, tested, and
wired to a dead input for weeks. This file tests the failure.

The assertion messages name the file, the line, the mechanism and the fix. That
is what an error message is for.

**One stale reference.** The header cites `mm_backtest.py:1209-1210` for the
depth plumbing. In the current engine (`55b239ed60d3`, 3,029 lines) that code is
at **2066–2071**; line 1209 is inside `_arrive`. The mechanism described is
exactly right — only the line numbers have drifted. **Cite the symbol, not the
line.**

### 38.4 What is held identical, and what is not

Held identical across both arms — the confirmed-edge production knobs:

```python
BASELINE = dict(obi_throttle=True, ofi_throttle=False,
                obi_throttle_thresh=0.15, throttle_frac=0.5,
                throttle_hold_ms=300.0)
```

All five match `micro_mm`'s own defaults and the production settings recorded in
§11 and §27A.6, so the *throttle* baseline is correct.

Run shape: universe from `mm_watchlist_final.csv`; `TRAIL_DAYS = 10` warm-up
dropped; `CLIP_MULT = 3.0`; `MAX_DAYS = 20` sampled evenly across the remaining
dates; `WORKERS = 9`; one process pool over dates, both arms and all symbols
inside each date so a day's datasets are opened once.

**Paths come from `config_pk`** — `from config_pk import PARSED_ROOT,
RESULTS_ROOT` — which is the thing §37.1 asks every script to do and
`build_feature_store.py` does not. Correct here.

### 38.5 The measurement

```python
g["bps"] = np.where(g["opn"] > 0, g["pkr"] / g["opn"] * 1e4, np.nan)
delta = (w_bps[arm] - w_bps[CONTROL]).dropna().to_numpy()
m = delta.mean(); se = delta.std(ddof=1)/np.sqrt(len(delta)); t = m/se
```

Daily portfolio P&L over daily opened notional, in bps — **a rate, not a
level**, which is §27A.8 lesson 1. Then the **paired daily difference on days
both arms produced**, day as the unit, which is §26.4 and §35 lesson 3. Both
correct.

**The verdict rule is pre-registered in code, before the run:**
...
A decision rule written down before seeing the number is the single most
effective guard against reading a table for its best cell. §27A.4 does the same
thing with its stated prior.

**Two supporting details that are right and easy to get wrong:**

- `_attr` **excludes end-of-day liquidation fills** before FIFO attribution —
  *"feeding them into `fifo_attribution` double-handles the haircut (the
  documented 1-2 PKR leak)"*. Net P&L stays the engine's own headline; only the
  **denominator** is built from intraday fills.
- `_safe_parquet` writes to a temp path, **re-opens it and asserts the row count
  matches**, then atomically replaces — with a CSV fallback that says so in the
  log. A write that is not read back is not a write.

The annualised Sharpe is flagged in the source as a proxy and points at
`sweep_risk_score.py` for the real set. Correctly labelled.

### 38.6 Four design criticisms, in order of how much they matter

**1. Twenty days cannot answer this question, and the arithmetic says so before
the run.** This is the one to fix.

From §20.1, the QT_2t-over-OBI paired edge is **+1.3036 bps/day at t = +9.42**
over the 197-date panel. That implies a standard error of 0.1384 and a **daily
paired standard deviation of 1.94 bps**. At that dispersion:

| days | smallest effect resolvable at \|t\| = 2 |
|---|---|
| **20 (as configured)** | **0.87 bps/day** |
| 60 | 0.50 |
| 100 | 0.39 |
| 197 (the full panel) | 0.28 |

Turned around: 60 days to resolve 0.50 bps/day, **168 days to resolve 0.30**,
and 377 to resolve 0.20.

**So the 20-day run can only detect an effect two-thirds the size of the biggest
signal improvement ever found in this project** — and the screen that motivated
it reports a standalone advantage of +0.003 partial-rho, which is not a
promising prior for an 0.87 bps/day effect. **With the pre-registered rule, the
overwhelmingly likely outcome is "FLAT → keep L1" whether or not the signal is
worth anything**, and that outcome is then indistinguishable from the silent-null
the file works so hard to prevent. The guards stop the *wiring* from faking a
null; they cannot stop the *sample size* from doing it.

Two concrete fixes, either of which is cheap:

- Run the smoke and the wiring check at 20 days, then **run the decision at the
  full panel**, as the QT_2t confirmation and the 110-name gate confirmation
  both were.
- Or print the **minimum detectable effect beside the verdict** — compute it
  from the run's own `delta.std(ddof=1)` — so "FLAT" reports what it could have
  seen. A negative result without its power is not a result.

**2. The baseline is not the shipped book — and this project has already learned
that lesson once.** §27A.8 item 8 records it from `extreme_obi_sweep.py`:

> *"A sweep that puts every name on one uniform lean is not measuring the thing
> that ships."*

`extreme_obi_sweep.py` reads `config_assignment_20260915_0043.csv`, sets **each
name's own lean** from it, and forces **KEL / PIBTL / TPL** to plain OBI because
the lean is destructive on a one-tick book (§20.1). `wobi_headtohead.py` does
neither: there is no `queue_skew_thresh` anywhere in it, no assignment read, and
no cheap-tick exclusion. Every name runs one uniform configuration.

That matters here specifically, not just in principle. The cheap-tick three are
the names where an imbalance signal behaves worst — capture went **+1.64 → −1.79
bps** on them — and a weighted signal that reaches to level 2 is *most* likely to
behave differently on a one-tick book, in either direction. Leaving them in on a
uniform setting either buries a real effect in their noise or manufactures one.

**Fix:** import `per_name_setting()` and `CHEAP_EXCLUDED` from the pattern
`extreme_obi_sweep.py` already established, rather than re-deriving a baseline.

**3. The universe is the watchlist, not the assignment.** `mm_watchlist_final.csv`
is the screening output (§33); the 113 names that ship come from the assignment
CSV. If those two lists differ, the head-to-head is measured on a population the
production book does not quote. **Print both counts at start-up and the
intersection**, or read the assignment.

**4. No run identity in the artifact.** Output is a fixed
`wobi_headtohead.parquet` / `.csv` with no tag. `extreme_obi_sweep.py` learned
this the hard way and now hashes **arms + names + dates + column schema**
(§27A.8 item 6), because a file that does not record what produced it reconciles
perfectly while being wrong. This script overwrites rather than appends, so it
cannot produce the ragged-union failure — but it also cannot tell you which arm
set the CSV on disk came from. **Write the arm dict, the date list and the engine
digest into the file.** §18.1 already requires the engine digest everywhere else.

**A fifth, minor:** `bucket="all"` is hardcoded, so there is no intraday
breakdown. A depth-weighted signal plausibly earns its keep only when the touch
is thin — early, late, or in wide-spread regimes — and a single pooled number
cannot see that. The time buckets already exist (§13); using them costs nothing.

### 38.7 What is established, and what is not

| claim | status |
|---|---|
| Decay-weighted OBI work **exists** in this project | **Established.** This file is it |
| It is judged **on money**, not on correlation alone | **Established**, and pre-registered |
| The weighting at depth 2 is `L1 + r·L2`, `r ∈ {0.3, 0.5}` | **Established** from the arm definitions |
| The general weighting form beyond depth 2 | **Not established** — read `_weighted_imb` |
| The screen's numbers (+0.023 incremental, +0.003 standalone) | **Quoted from this file's header.** `obi_decay_gate` not read |
| Any **result** of the head-to-head | **None exists anywhere I can see.** No output file, no comment, no recorded verdict |
| The engine supports `weighted_obi` | **Not in any copy I hold.** See below |

**The engine dependency, stated precisely.** The file says it *"REQUIRES the
`micro_mm.py` weighted_obi edits (`weighted_obi` / `wobi_depth` / `wobi_decay`
kwargs + `_weighted_imb`)"*. Searching every copy of `micro_mm.py` supplied in
this project — `362f8a5b429d` (the newest, 1,994 lines), `b02d4c264486` and
`bb07a17ec111` — **returns zero occurrences of `weighted_obi`.** `quotes()` does
take `depth=None`, so the plumbing half exists; the weighting itself does not.

So one of two things is true, and the file cannot tell me which:

1. SZ has a newer `micro_mm.py` with the edits, not yet supplied here; or
2. the edits are still to be made, and `wobi_headtohead.py --run` would raise
   `TypeError` on the first weighted arm.

The file is explicit that (2) fails **loudly**, which is the right design.
**This is the first thing to check before running it**, and it is a one-liner —
the same probe `extreme_obi_sweep.py` uses for its own missing kwarg:

```python
"weighted_obi" in MicrostructureMM.__init__.__code__.co_varnames
```

**Suggestion:** add that probe to `validate_arms()`. It currently guards the
depth plumbing but not the kwarg's existence, and the failure it would catch is
the one the file itself warns about.

### 38.8 What this changes elsewhere in this document

| section | was | now |
|---|---|---|
| **§27.2** | "no decay sweep in any file read; candidates are `expand_feature_store.py`, `probe_conditional_markout.py`" | **Wrong. It exists** — `obi_decay_gate` (the screen) and `wobi_headtohead.py` (the money test). The remaining unread file is `obi_decay_gate` itself |
| **§37.2** | "There is no per-level decay and no decay sensitivity sweep in any file read for this document" | **Withdrawn.** True of `build_feature_store.py`, `extreme_obi_sweep.py` and the engine; false of the project |
| **§34 row 1** | the open gap | **Closed as an existence question**, reopened as a *result* question: the work exists, the answer does not |
| **§37.4** | eleven corrections | **twelve** — this is #12, and the largest |

**Why the earlier conclusion was wrong, which is the part worth keeping.** §27.2
and §37.2 were carefully hedged — "in any file I have read", with the unread
candidates named. The hedge was honest and the conclusion still misled, because
the *emphasis* did not match the *evidence*: "on the evidence now in hand, no
per-level decay exists in this codebase" reads as a finding about the project
when it was only a finding about my file list. **An absence of evidence stated
with the confidence of evidence of absence is a claim, and it was the wrong
one.** The fix is not more hedging — it is to say what was searched, and to stop
there.

---

# PART V — ADDED 2026-09-18 (the corrected-engine re-run)

The 113-name re-run under the corrected engine finished. **Every headline number
in §20.1 and §20.2 moves**, and one of the moves is large enough that it is
recorded here as **unexplained** rather than explained.

---

## 39. THE 113-NAME RE-RUN UNDER THE CORRECTED ENGINE

### 39.1 Run identity

| | |
|---|---|
| scope | **113 names × 197 dates × 3 arms = 66,783 cells**, 0 resumed from journal |
| wall clock | **600m31s**, 9 workers |
| dates | 2025-09-15 → 2026-06-30 |
| engine | `mm_backtest.py` **sha1 d4999ce5b5fa** = md5 `55b239ed60d3` (the corrected engine: crossing-on-arrival, price band, the three counters) |
| strategy | `micro_mm.py` **sha1 b8b1262c30d2** = md5 `362f8a5b429d` |
| tag | `f653565c` = calibration + arms + engine |
| calibration | `session_scales_20260913_0957`, `volume_profile_20260913_1234`, `time_windows_20260913_1245`, `session_segments_20260820_0107` |
| anchors | **0 PKR unexplained on all three arms, 0.0%** |

**Both engine digests were verified independently**: the run prints sha1, and
sha1 of the two files this document describes reproduces them exactly. This is
the engine-digest-in-the-journal discipline of §18.1 doing its job — the run
proves which engine produced it, and a reader can check.

**One stale line in the log to ignore.** The banner reads *"FULL-YEAR
CONFIRMATION — OBI vs QT_1t vs QT_2t vs QBPS_2 … OFF vs ON = 3 configs"*. That
is boilerplate carried from the original sweep. The arms actually in force are
printed two lines above it and are **OBI, QT_2t@15, QT_2t@20**. There is no
QT_1t or QBPS_2 in this run.

### 39.2 The headline

| arm | net bps | net PKR | mean bps/day | se | Sharpe | Sortino | maxDD PKR | win% |
|---|---|---|---|---|---|---|---|---|
| OBI (lean off) | 0.715 | **1,457,878** | 0.789 | 0.229 | 3.9 | 2.9 | **−535,337** | 70% |
| QT_2t@15 | 2.419 | **9,920,763** | 2.465 | 0.149 | 18.7 | 15.9 | −122,598 | 88% |
| QT_2t@20 | 2.519 | **9,925,458** | 2.545 | 0.151 | **19.1** | 16.6 | **−119,106** | 87% |

**The risk result is larger than the P&L result and is new.** The lean does not
just earn more, it earns it far more smoothly: **Sharpe 3.9 → 18.7, and maximum
drawdown improves 4.4×, from −535,337 to −122,598 PKR.** Nothing in §20
recorded that, because the earlier runs did not compute drawdown.

**Paired, day as the unit** (reproduced from the run's own DAILY parquet):

| comparison | bps/day | se | t | n | daily paired sd |
|---|---|---|---|---|---|
| QT_2t@15 − OBI | **+1.6769** | 0.1376 | **+12.19** | 197 | 1.931 |
| QT_2t@20 − OBI | **+1.7562** | 0.1337 | **+13.14** | 197 | 1.876 |
| **QT_2t@20 − QT_2t@15** | **+0.0793** | 0.0261 | **+3.03** | 197 | **0.367** |

**The lean's edge grew under the correction**: §20.1 recorded +1.3036 bps/day at
t = +9.42; it is now **+1.677 at t = +12.19**.

**The third row is not in the run's own log and is the one that changes a
decision.** Uniformly, trigger **0.20 beats trigger 0.15 at t = +3.03** — clearing
the |t| > 2 bar this project applies everywhere. Note what makes that possible:
the two lean arms move together day to day, so the **paired** sd is **0.367 bps
against 1.9** for either arm against OBI. A small effect is measurable when the
two things being compared are alike.

**Do not read that as "ship 0.20 everywhere."** §20.2's t = +1.64 was a
different test — the walk-forward gain of the *three-bucket assignment rule*
over the two-bucket rule, out of sample. This is *uniform 0.20 vs uniform 0.15*,
in sample, on one window. The honest statement is: **uniform 0.20 is better than
uniform 0.15 in sample, and whether the per-name split beats uniform 0.20 is a
walk-forward question this run does not answer.** It should be asked at the next
refit, because it is now a live possibility that the whole three-bucket
apparatus is beaten by one number.

### 39.3 The bucket decomposition — where the lean actually works

Net bps by time bucket:

| bucket | OBI | QT_2t@15 | QT_2t@20 |
|---|---|---|---|
| first15 | **−0.460** | +1.449 | +1.456 |
| middle | +1.752 | +2.756 | **+2.915** |
| preclose45 | **−2.483** | +2.016 | +1.971 |
| last15 | **−2.972** | +0.499 | +0.405 |

**Without the lean the book loses money in three of the four buckets and is
carried entirely by the middle of the day.** With it, all four are positive.
The largest single improvement is `preclose45`: **−2.483 → +2.016 bps**, a swing
of 4.5 bps in the bucket where inventory pressure and the exit ramp bite hardest.

**The mechanism is visible in the signed OBI column**, and this is the cleanest
demonstration of the lean in the whole project:

| bucket | signed obi5, OBI arm | signed obi5, QT_2t@15 |
|---|---|---|
| first15 | −0.0249 | **+0.0170** |
| middle | −0.0145 | **+0.0368** |
| preclose45 | −0.0191 | **+0.0421** |
| last15 | −0.0217 | **+0.0227** |

Negative means the book was leaning against us when we got filled. **The sign
flips in every bucket.** Without the lean we are systematically filled on the
wrong side of the imbalance; with it, on the right side. That is the entire
thesis of §11, measured directly.

**And the lean's edge is capture, not markout.** Daily markout is *more*
negative with the lean on (mean −45,068 PKR/day vs −33,853, both t ≈ −13). The
lean takes **more** adverse selection and wins anyway, because it earns more
spread. A reader who optimises markout would switch the lean off and lose money.

`liq_bps` is the other thing worth noting: **−3.026 bps in `last15` on the OBI
arm** and −1.912 with the lean. End-of-day liquidation is a real and large cost
concentrated in the final bucket, and the lean roughly halves it.

### 39.4 THE 25% LEVEL SHIFT — measured, attributed, and reconciled with the gate

**This was written up as unexplained. It is now explained, and the explanation
invalidates how §20.4's headline figure was used.**

Comparing per name against the shipped assignment table
(`config_assignment_20260915_0043.csv`), which was built from the
**pre-correction** engine on **the same names and the same dates** (days per
name: old min/median/max **196/196/197**, new **196/196/197**):

| arm | old total PKR | new total PKR | change | names up | names down |
|---|---|---|---|---|---|
| OBI | 4,234,221 | 1,457,878 | **−65.6%** | 13 | **100** |
| QT_2t@15 | 13,285,941 | 9,920,763 | **−25.3%** | 13 | **100** |
| QT_2t@20 | 13,360,332 | 9,982,453 | **−25.3%** | 10 | **100** |

Only 2–3 names of 113 land within ±1% of their old figure. **This is a
systematic level shift across the whole book, not a few names moving.**

**It conflicts with §20.4 by two orders of magnitude.** The reconcile gate
measured the same two model corrections at **−82.08 PKR, −0.27%**, on 12 names ×
20 June dates. This run shows −25% on 113 names × 197 dates. Both cannot be a
measure of the same quantity.

#### The attribution: it is entirely CAPTURE

The old cohort runs (`universe_expand_PERNAME_20260913_1818` = the 38 names,
`…_1253` = the 75) carry the full component breakdown, so old and new can be
decomposed side by side on the same 22,204 symbol-days. **In rates, per bps of
opened notional:**

| component | OBI old → new | QT_2t@15 old → new |
|---|---|---|
| **capture** | **7.762 → 6.420 (−1.342)** | **7.670 → 6.597 (−1.072)** |
| markout | −3.281 → −3.271 (+0.010) | −2.386 → −2.165 (**+0.221**) |
| fees | 1.554 → 1.554 (0.000) | 1.554 → 1.554 (0.000) |
| liquidation | −0.794 → −0.881 (−0.087) | −0.415 → −0.460 (−0.045) |
| **net** | **2.134 → 0.715** | **3.316 → 2.419** |

Also: **opened notional rose** (+2.8% / +2.4%) while **fill count fell**
(−4.3% / −3.1%).

**Capture moved and markout did not. That is the whole finding**, and it is the
§27A.1 signature exactly: *"Negative capture is not adverse selection. Adverse
selection shows up in markout. Negative capture means the quote was filled on the
wrong side of the mid at the instant of the fill."* A crossing arrival pays
through the spread — the damage is booked at the fill, and there is no extra
drift afterwards. Markout on the lean arm even **improves**, because the adverse
move that made the order crossing has already happened by the time it fills.

Fees per bps are **identical to three decimals**, which rules out any change to
the fee schedule, the clip or the calibration as the cause.

**It is broad, not concentrated: 108 of 113 names have a worse capture rate**,
median −0.983 bps, and the damage is larger on the cheap-tick names — WTL
(+1.362 → −13.854), KEL, CNERGY, KOSM, PIBTL head the list, and
`corr(log traded notional, capture change) = +0.437`: **the bigger the name, the
smaller the hit.** That is what a book-walking cost looks like.

#### Why the gate said −0.27% — `gate.py` line 522

The gate does not run the production book. It runs:

```python
params = H.build_micro_params(50, scales[sym], profiles[sym], windows[sym], segs)
```

**A hardcoded 50-share clip.** Production is `CLIP_MULT = 3.0 ×` the trailing
median trade size; this run's own log reports **366–525 shares per trade**.

That is the entire reconciliation, and it is a mechanism, not a coincidence:
**the cost of crossing is convex in order size.** A 50-share order that arrives
marketable is filled inside the **top level** of the opposite book and pays one
tick of damage. A 450-share order **walks down the levels**, paying worse at
each. The gate's own counters confirm the first half directly:
**58,648 shares crossed over 1,284 crossing orders = 45.7 shares per crossing
order**, against a 50-share clip — essentially every one filled at the touch.

The same correction, on the **same 12 names and the same 20 June dates**, at the
two order sizes:

| configuration | before | after | change |
|---|---|---|---|
| gate, 50-share clip | 30,785.24 | 30,703.16 | **−0.27%** |
| universe run, production clip, QT_2t@15 | 332,906 | 297,423 | **−10.7%** |
| universe run, production clip, OBI | 106,320 | 76,062 | **−28.5%** |

And the gate is running the **lean-off** configuration: its 16,527 fills match
the universe run's OBI arm on those cells (**16,509**) to 0.1%, not the lean
arm's 36,539.

**Both numbers were right. The error was mine, in treating them as measurements
of the same quantity.** §20.4's −82.08 PKR is what the venue corrections cost a
50-share book on 12 names in June. It was never an estimate of what they cost
the production book, and this document previously presented it as one.

#### The rule this establishes

**The reconcile gate is a correctness harness, not a P&L harness.** It proves
two engines reproduce each other to the paisa. It **cannot** size the P&L effect
of any venue rule whose cost depends on order size — crossing, the price band,
queue position, anything that walks the book — because it runs a clip an order
of magnitude below production. Any such estimate must come from a run at
production size.

**What remains open** is narrow: the residual difference between the gate's
30,703 PKR and the universe run's 76,062 on the same cells and near-identical
fill counts, which is a size effect that is **sublinear** — a 9× clip does not
earn 9× the money, because a larger quote does not fill proportionally more
often. That is expected and is not a discrepancy, but it has not been measured.

### 39.5 The assignment rebuilt — the reassuring half

Replicating `build_config_assignment.py`'s rule exactly (effect size
`d = mean/sd` of the daily `lean 0.15 − lean off` series, `D_BAND = 2/√98 =
0.202`, `D_EXIT = D_BAND/2`, hysteresis against the shipped assignment, the
capacity flag, the chosen-setting gate, `T_REENTRY = 2.0`) on the corrected-engine
data:

| | QT_2t@15 | QT_2t@20 | OBI | DROP |
|---|---|---|---|---|
| shipped | 68 | 13 | 17 | 15 |
| **rebuilt** | **70** | **9** | **16** | **18** |

**Only 6 of 113 names change bucket:**

| name | shipped | → new | d_eff | reason |
|---|---|---|---|---|
| HASCOL | OBI | **DROP** | −0.327 | loses under every setting |
| WTL | OBI | **DROP** | −0.095 | loses under every setting |
| TREET | QT_2t@20 | OBI | −0.051 | in band; off beats 0.20 |
| SGF | QT_2t@20 | **DROP** | +0.115 | loses under every setting |
| BOP | QT_2t@20 | QT_2t@15 | +0.221 | d clears the band |
| IMAGE | QT_2t@20 | QT_2t@15 | +0.221 | d clears the band |

**The effect sizes are almost perfectly preserved: correlation between old and
new `d_eff` is +0.980 across the 110 names where both exist** (old mean +0.313,
new +0.286). So the −25% level shift is close to **multiplicative and common
across names** — it moves every name's P&L down without reordering which names
the lean helps. That is why an assignment rule built on a *within-name
difference* is almost unaffected by it, and it is a good argument for building
decision rules on paired differences rather than levels.

**Totals on the corrected engine (197 days, in sample):**

| | PKR |
|---|---|
| rebuilt assignment | **12,568,176** |
| shipped assignment applied to the new data | 12,520,169 |
| everyone on lean 0.15 | 9,920,763 |
| everyone on lean 0.20 | 9,925,458 |
| per-name ceiling (best of three, or drop to zero) | 12,776,468 |

**Re-running the assignment buys +48,007 PKR, +0.38%.** The shipped assignment,
built on the old engine, is still within half a percent of the best in-sample
assignment under the new one. **There is no urgency to refit.** The rebuilt
assignment captures **98.4%** of the per-name ceiling and beats flat-0.15 by
**+26.7%** — both close to the §20.2 figures once §20.2's percentage is corrected
to 98.8% (§37.4 #8).

**Caveat, stated because it is the same one §20.2 carries:** this is one
in-sample window, not a walk-forward. It answers *"does the assignment change
under the corrected engine"* — no, barely — and not *"is the new assignment
better."*

### 39.6 A correction to §38.6, from this run's own data

§38.6 argued that a 20-day paired test of decay-weighted OBI could only resolve
**0.87 bps/day**, derived from a daily paired sd of 1.94 bps. **That sd was taken
from the wrong comparison, and the criticism was therefore too harsh.**

This run measures both regimes directly:

| paired comparison | daily paired sd | 20-day minimum detectable effect at \|t\|=2 |
|---|---|---|
| lean **on vs off** (very different arms) | 1.93 bps | 0.86 bps/day |
| trigger **0.15 vs 0.20** (nearly identical arms) | **0.367 bps** | **0.16 bps/day** |

A weighted-versus-plain imbalance is a *small* change to the same mechanism, so
it belongs in the second row, not the first. **At a paired sd near 0.37, twenty
days resolves about 0.16 bps/day**, which is a reasonable test.

The structural point in §38.6 stands and is strengthened: **the minimum
detectable effect must be computed from the paired difference's own sd and
printed beside the verdict.** `wobi_headtohead.py` already computes
`delta.std(ddof=1)`; it is two lines to print `2 × se` next to the "FLAT"
reading. What was wrong was picking the sd from an unrelated pair — which is the
same error as quoting a number without recomputing it (§35 lesson 26).

### 39.7 What this run does not settle

1. ~~The −25% level shift~~ — **closed in §39.4.** It is crossing-on-arrival,
   measured as a pure capture effect, and the gate's −0.27% was a 50-share clip.
   What remains is to confirm it directly with a `cross_on_arrival=False` run on
   the ten largest movers, which should now be a formality.
2. **Uniform 0.20 vs the three-bucket assignment**, out of sample. Uniform 0.20
   now beats uniform 0.15 at t = +3.03, which was not true before.
3. **Market impact.** Every number here assumes my orders do not move the price.
   That assumption has never been measured on this book, and it is the one that
   binds hardest on the thin names where participation is highest. It is being
   measured separately; until it is, the per-name P&L on small names should be
   treated as an upper bound.
4. **Whether `preclose45` and `last15` should be quoted at all on the OBI names.**
   Those buckets lose 2.5–3.0 bps without the lean, and 17 names ship on OBI.
   The bucket split exists; nobody has tested switching those names off late in
   the day.
