...
### 39.4 THE UNEXPLAINED RESULT — a 25% level shift that the gate did not predict

**This is the most important thing in this section and it is not resolved.**
...
**A hypothesis that fits the data, labelled as a hypothesis.** Crossing-on-arrival
converts orders that used to be *rejected* into orders that *execute at adverse
prices*. How often that happens depends on how close the quote sits to the
opposite touch — so it should hurt the **no-lean** arm most, because the lean's
whole job is to push the quote away from the side the book is leaning toward.
The data is consistent with that: **OBI fell 65.6% and the lean arms 25.3%**, and
the lean's measured edge *grew* from 1.30 to 1.68 bps/day. The old engine's own
docstring even estimates the frequency on one liquid name — *"48 of 2,673 sends
on the sample day"*, 1.8% — and a book of 113 names including thin ones would
cross far more often than a gate sample of 12.

**But that is reasoning, not measurement, and it must not be written down as the
answer.** Two other candidates are not excluded: the price-band clamp now binds
where it previously did not, and the old figures come from **three separate
runs** (the 38-name and 75-name cohorts plus the 110-name `gate20` confirmation)
stitched together, which may not be a like-for-like panel however well the day
counts match.

**The decisive diagnostic already exists and takes one run.** The corrected
engine carries `cross_on_arrival` as a config flag, put there specifically to
reproduce a pre-correction number, and three counters — `crossed_on_arrival`,
`crossed_on_arrival_shares`, `crossed_on_arrival_rested`.

1. Re-run a **small panel** — the ten largest movers (NRL −218,923, NPL −119,968,
   HASCOL −104,404, PTC −96,286, ATRL −89,296, …) over the same 197 dates — with
   `cross_on_arrival=False`.
2. If the old totals come back, the correction is the whole story and the gate's
   −0.27% was simply a sample that does not cross much.
3. If they do not, something else changed between the old runs and this one, and
   **no number in §20 is comparable to any number in §39 until that is found.**
4. Either way, report the per-arm `crossed_on_arrival` counts. If OBI crosses
   materially more often than QT_2t@15, the hypothesis above is measured rather
   than argued.

**Until that runs, treat §20's PKR levels and §39's PKR levels as belonging to
two different engines and do not mix them in any table.** The *paired* and
*per-name-ranking* results survive either way — see below — because they are
differences measured within one engine.
...
1. **The −25% level shift** (§39.4). One small `cross_on_arrival=False` run
   closes it. Nothing in §20 should be quoted alongside §39 until it does.
