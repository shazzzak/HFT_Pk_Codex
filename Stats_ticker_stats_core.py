...
def stats_for_symbol(snap, trades, symbol):
    ...
    # volume that traded while spread was already wider than round-trip fee
    tcv["rt_net"] = tcv["prev_spread"] - 2 * FEE_TOTAL_PCT * tcv["price"]
    qual = tcv[tcv["rt_net"] > 0]
    row["qualifying_vol_sh"] = float(qual["qty"].sum())
    row["qualifying_notional_m"] = float((qual["qty"] * qual["price"]).sum() / 1e6)
    row["pct_vol_qualifying"] = (float(qual["qty"].sum()) / total_vol) if total_vol > 0 else 0.0
    # ceiling: perfect capture of half the prevailing spread on qualifying vol,
    # /2 because a round trip consumes two fills
    row["ceiling_pkr"] = float((qual["rt_net"] * qual["qty"]).sum() / 2)
    return row
